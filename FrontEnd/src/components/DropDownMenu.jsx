...
function DropDownMenu({ onChange }) {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-7 sm:px-6 sm:py-10 lg:max-w-7xl lg:px-8">
      ...
      <Label className="block text-sm font-medium leading-6 text-gray-900">Ha seleccionado filtrar por la profesion:</Label>
      <div className="relative mt-2">
        ...
        >
          {profesions.map((prof) => (
            <ListboxOption
              key={"k"+prof.name+prof.id}
              value={prof}
              className="group relative cursor-default select-none py-2 pl-3 pr-9 text-gray-900 data-[focus]:bg-indigo-600 data-[focus]:text-white"
            >
              <div className="flex items-center">
                <img alt="" src={prof.img} className="h-5 w-5 flex-shrink-0 rounded-full" />
